...
if (!class_exists('jobcareer_mailchimp')) {
	...
	public function __construct() {
	    ...
	    parent::__construct(
		    ...
		    array('classname' => 'widget-newsletter fancy', 'description' => esc_html__('Mail Chimp Newsletter Widget', 'jobcareer'),) // Args
	    );
	}
	...
	function form($instance) {
		...
		'desc' => '',
		'hint_text' => esc_html__("Enter your widget title here.", 'jobcareer'),
		'echo' => true,
		'field_params' => array(
		    'std' => esc_attr($title),
		    'cust_id' => '',
	...
	function widget($args, $instance) {
		...
		<div class="fieldset">
			...
			$jobcareer_form_fields->cs_form_text_render($cs_opt_array);
			if (isset($select_view) && $select_view == 'classic') {
			    $cs_opt_array = array(
				'std' => '',
			    ...
			    <?php // $jobcareer_form_fields->cs_form_text_render($cs_opt_array); ?>
		    				    </label>-->
		    	<label>
		    		<!--<i class="icon-location7"></i>-->
			    ...
			    <?php
			} elseif (isset($select_view) && $select_view == 'fancy') {
			    ?>

		    	<button type="submit" class="fancy" value="Submit"><i class="icon-angle-right"></i></button>
...
add_action('widgets_init', function() {
    return register_widget("jobcareer_mailchimp");
});
